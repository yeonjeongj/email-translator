...
import type { Company, Thread, Message, Lang, Summary } from "@/lib/types";
import type { TargetLanguage } from "@/lib/translate";

interface Props {
  open: boolean;
  onClose: () => void;
  onSaved: (company: Company, thread: Thread) => void;
  onSummaryError?: () => void;
  sourceText: string;
  translatedText: string;
  targetLang: TargetLanguage;
  replySourceText?: string;
  replyTranslatedText?: string;
  replyTargetLang?: TargetLanguage;
}
...
const LANG_MAP: Record<TargetLanguage, Lang> = {
  KO: "ko",
  EN: "en",
  JA: "ja",
  ZH: "zh",
...
export default function SaveModal({
  ...
  async function handleSave() {
    ...
    const outboundMessage: Message | null =
      replySourceText?.trim() && replyTranslatedText?.trim() && replyTargetLang
        ? {
            id: crypto.randomUUID(),
            direction: "outbound",
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-outline-variant flex justify-between items-center">
          <h2 className="font-semibold text-on-surface text-base">번역 저장</h2>
          <button
            onClick={onClose}
